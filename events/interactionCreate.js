...
const postCommentsPath = path.join(__dirname, '../post_comments.json');
    ...
    async execute(interaction) {
        try {
            ...
            if (interaction.isModalSubmit()) {
                if (interaction.customId.startsWith('modal_comment_post_')) {
                    const postId = interaction.customId.replace('modal_comment_post_', '');
                    const comentario = interaction.fields.getTextInputValue('comment_input')?.trim();

                    if (!comentario) {
                        return interaction.reply({
                            content: '❌ Escreva um comentário válido.',
                            ephemeral: true
                        });
                    }

                    const postComments = lerJson(postCommentsPath);
                    ...
                    postComments[postId].comments.push({
                    ...
                    salvarJson(postCommentsPath, postComments);

                    const totalComentarios = postComments[postId].comments.length;

                    const postLikes = lerJson(postLikesPath);
                    const totalLikes = postLikes[postId]?.userIds?.length || 0;

                    const row = new ActionRowBuilder().addComponents(
                        new ButtonBuilder()
                            .setCustomId(`like_post_${postId}`)
                            .setLabel(`❤️ ${totalLikes}`)
                            .setStyle(ButtonStyle.Danger),

                        new ButtonBuilder()
                            .setCustomId(`comment_post_${postId}`)
                            .setLabel(`💬 ${totalComentarios}`)
                            .setStyle(ButtonStyle.Primary),

                        new ButtonBuilder()
                            .setCustomId(`viewcomments_post_${postId}`)
                    ...
                    setTimeout(() => {
                        interaction.webhook.deleteMessage(replyMsg.id).catch(() => {});
                    }, 25000);
            ...
            if (interaction.customId.startsWith('like_post_')) {
                ...
                const totalLikes = postLikes[postId].userIds.length;
                const totalComentarios = postComments[postId]?.comments?.length || 0;
                ...
                let lista = 'Ninguém curtiu esta postagem ainda.';
                if (postLikes[postId].userIds.length > 0) {
                    ...
                    const validos = users.filter(Boolean);

                    if (validos.length > 0) {
                        lista = validos.map(u => `• ${u.tag}`).join('\n');
                    }
                }

                const msg = await interaction.followUp({
                    content: `❤️ **Curtidas nesta postagem:**\n\n${lista}`,
                    ephemeral: true,
                    fetchReply: true
                });
            ...
            // =========================
            if (interaction.customId.startsWith('comment_post_')) {
                const postId = interaction.customId.replace('comment_post_', '');

                const modal = new ModalBuilder()
                    .setCustomId(`modal_comment_post_${postId}`)
                ...
                const input = new TextInputBuilder()
                    ...
                    .setRequired(true)
                    .setMaxLength(300)
                    .setPlaceholder('Digite aqui seu comentário...');
            ...
            // VER COMENTÁRIOS
            // =========================
            if (interaction.customId.startsWith('viewcomments_post_')) {
                const postId = interaction.customId.replace('viewcomments_post_', '');
                const postComments = lerJson(postCommentsPath);

                if (!postComments[postId] || !postComments[postId].comments.length) {
                    const msg = await interaction.reply({
                        content: '💬 Esta postagem ainda não possui comentários.',
                        ephemeral: true,
                        fetchReply: true
                    });
                ...
                const comentarios = postComments[postId].comments
                    .slice(-10)
                    .map(c => `**${c.username}**: ${c.content}`)
                    .join('\n\n');

                const msg = await interaction.reply({
                    content: `💬 **Últimos comentários:**\n\n${comentarios}`,
                    ephemeral: true,
                    fetchReply: true
                });
            ...
            if (interaction.customId.startsWith('aprovar_post_')) {
                ...
                try {
                    ...
                    const rowInteracoes = new ActionRowBuilder().addComponents(
                        new ButtonBuilder()
                            .setCustomId(`like_post_${postId}`)
                            .setLabel('❤️ 0')
                            .setStyle(ButtonStyle.Danger),
